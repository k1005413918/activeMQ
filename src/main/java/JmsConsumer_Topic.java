        ...
        messageConsumer.setMessageListener(new MessageListener() {
           ...
           public void onMessage(Message message) {
               if (message != null && message instanceof TextMessage) { //对象instanceof类,对象是后面类或者其子类的实例 返回true
                   ...
                   try {
                       System.out.println("****消费者接受到的text消息："+textMessage.getText());
                   } catch (JMSException e) {
                       e.printStackTrace();
                   }
               }
               if (message != null && message instanceof MapMessage) { //对象instanceof类,对象是后面类或者其子类的实例 返回true
                   MapMessage mapMessage = (MapMessage) message;
                   try {
                       System.out.println("****消费者接受到的map消息："+mapMessage.getString("k1"));
                       System.out.println("****消费者接受到的map消息属性："+mapMessage.getStringProperty("StringProperty"));
                   } catch (JMSException e) {
                       e.printStackTrace();
                   }
               }
